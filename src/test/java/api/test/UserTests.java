...
public class UserTests {
	...
	public void test_PostUser()
	{
		...
		Response response= UserEndPoints.createUser(userPayload); //calling static method using classname directly and passing argument
	...
	public void test_getUserByName() 
	{
		...
		Response response=UserEndPoints.readUser(this.userPayload.getUsername()); //why this keyword?
	...
	public void test_updateUserByName() 
	{
		...
		Response response = UserEndPoints.updateUser(this.userPayload.getUsername(), userPayload);//why this keyword
		response.then().log().all();
		//response.then().log().body();
		AssertJUnit.assertEquals(response.getStatusCode(), 200);
		...
		Response responseAfterUpdate=UserEndPoints.readUser(this.userPayload.getUsername()); //why this keyword?
		response.then().log().all();
		//response.then().log().body();
	...
	public void test_deleteUserByName() {
		...
		Response response=UserEndPoints.deleteUser(this.userPayload.getUsername()); //why this keyword?
		response.then().log().all();
		AssertJUnit.assertEquals(response.getStatusCode(),200);
		//response.then().log().body();
